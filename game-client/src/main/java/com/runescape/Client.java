...
public class Client extends GameApplet {
		...
		do {
			...
			if (openInterfaceId != -1 && openInterfaceId == reportAbuseInterfaceID) {
				...
				if (key == 13 || key == 10) {
					if (amountOrNameInput.length() > 0) {
						try {
							sendPacket(new SendAmount(Integer.parseInt(amountOrNameInput)));
						} catch (Exception ignored) {
	...
	private void drawLoginScreen() {setupLoginScreen();
		...
		if(registeringAccount) {
			...
			//Hovers
			usernameInputHover = mouseInRegion(drawXStart, drawYStart, drawXStart + 348, drawYStart + 32);
			passwordInputHover = mouseInRegion(drawXStart, drawYStart + 70, drawXStart + 348, drawYStart + 102);
			rememberMeHover = mouseInRegion(drawXStart, drawYStart + 107, drawXStart + 120, drawYStart + 120);
			loginHover = mouseInRegion(drawXStart + 245, drawYStart + 110, drawXStart + 345, drawYStart + 145);

			//Draw login box
			cacheSprite[340].drawAdvancedSprite(0, 0);

			//Draw username text box
			cacheSprite[usernameInputHover ? 342 : 341].drawAdvancedSprite(drawXStart, drawYStart);
			//Username
			newBoldFont.drawBasicString(myUsername, drawXStart + 10, drawYStart + 21, 0xD3D3D3);
			if((loginScreenCursorPos == 0) & (tick % 40 < 20)) {
				newBoldFont.drawBasicString(myUsername + "|", drawXStart + 10, drawYStart + 21, 0xD3D3D3);
			}

			//Draw password text box
			cacheSprite[passwordInputHover ? 344 : 343].drawAdvancedSprite(drawXStart, drawYStart + 72);
			//password
			String password = StringUtils.passwordAsterisks(myPassword);
			newBoldFont.drawBasicString(password, drawXStart + 10, drawYStart + 95, 0xD3D3D3);
			...
			//Remember me button
			if(rememberMe) {
				cacheSprite[rememberMeHover ? 346 : 348].drawAdvancedSprite(drawXStart + 6, drawYStart + 107);
			} else {
				cacheSprite[rememberMeHover ? 345 : 347].drawAdvancedSprite(drawXStart + 6, drawYStart + 107);
			}

			//Login button
			cacheSprite[loginHover ? 350 : 349].drawAdvancedSprite(drawXStart + 245, drawYStart + 110);

			//Draw errors
			int errorY = drawYStart + 168;
			if(firstLoginMessage.length() > 0) {
				newBoldFont.drawBasicString(firstLoginMessage, drawXStart + 2, errorY, 0xfff600);
				errorY += 22;
			}
			if(secondLoginMessage.length() > 0) {
				newBoldFont.drawBasicString(secondLoginMessage, drawXStart + 2, errorY, 0xfff600);
			}
