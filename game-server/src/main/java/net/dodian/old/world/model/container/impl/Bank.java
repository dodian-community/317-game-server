...
public class Bank extends ItemContainer {
	...
	public Bank open() {
		...
		getPlayer().setEnterSyntax(null);

		//Sort and refresh items in the container
		sortItems().refreshItems();

		//Send configs
		getPlayer().getPacketSender().sendConfig(115, getPlayer().withdrawAsNote() ? 1 : 0).sendConfig(304, getPlayer().isInsertMode() ? 1 : 0).sendConfig(117, getPlayer().isSearchingBank() ? 117 : 0).sendInterfaceSet(5292, 5063);
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		ItemDefinition def = definitionsManager.getItemDefinitionById(item.getId() + 1);
		if(def == null) {
			return this;
		}
	...
	public static int getTabForItem(Player player, int itemID) {
		if(Server.getDefinitionsHandler().getItemDefinitionById(itemID).isNoted()) {
