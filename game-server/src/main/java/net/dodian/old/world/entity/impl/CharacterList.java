...
public final class CharacterList<E extends Character> implements Iterable<E> {
	...
	/**
	 * This collections characters in a ArrayList which is shuffled
	 * every once in a while.
	 * Used for battling PID.
	 */
	private final List<E> shuffle_list = new CopyOnWriteArrayList<E>();
	 ...
	 */
	public void updateShuffleList(boolean shuffle) {
		shuffle_list.clear();
		Arrays.stream(characters).filter(e -> !Objects.isNull(e) && e.isRegistered()).forEach(e -> shuffle_list.add(e));
		if(shuffle) {
